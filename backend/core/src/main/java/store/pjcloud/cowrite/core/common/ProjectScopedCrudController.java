...
public abstract class ProjectScopedCrudController<T extends ProjectScoped> {
    protected final ProjectScopedCrudService<T> service;

    protected ProjectScopedCrudController(ProjectScopedCrudService<T> service) {
        this.service = service;
    }

    @GetMapping
    public List<T> findAll(@RequestParam(value = "projectId", required = false) UUID projectId) {
        UUID userId = SecurityUtils.requireUserId();
        return service.findAllForUser(userId, projectId);
    }
